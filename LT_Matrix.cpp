...
void Matrix<T> :: store(int i, int j, T value) const{
    ...
    this->element[((i*(i-1))/2)+j-1] = value;     // Row wise
    // return this->element[((j*(j-1))/2)+i-1];     // column wise
...
T Matrix<T> :: retrive(int i, int j) const{
    ...
    if (i>j)
    return 0;
    this->element[((i*(i-1))/2)+j-1];     // Row wise
    // return this->element[((j*(j-1))/2)+i-1];     // column wise
...
{
    cout<<"Enter the element of lower triangular matrix only ";
    for (int i = 0; i < (rows*(rows+1))/2; i++)
                in >> obj.element[i];
    return in;
}
